...
from subnet_management import validate_subnet, save_subnet_to_csv, load_existing_subnets, get_subnet_by_name
...
# Create server and ccd directories if they don't exist
        ...
        while True:
            openvpn_tunnel_subnet_input = input("Enter the OpenVPN tunnel subnet (e.g., 10.8.0.0/24): ")
            try:
                openvpn_tunnel_subnet = validate_subnet(openvpn_tunnel_subnet_input, existing_subnets)
                break
            except ValueError as e:
                print(f"Error: {e}")
        # Save to CSV
        save_subnet_to_csv(os.path.join(BASE_DIR, "subnets.csv"), "openvpn_tunnel_subnet", openvpn_tunnel_subnet)
        existing_subnets.append(str(openvpn_tunnel_subnet))

        # Prompt for Server LAN Subnet
        while True:
            server_lan_subnet_input = input("Enter the Server LAN subnet (e.g., 192.168.1.0/24): ")
            try:
                server_lan_subnet = validate_subnet(server_lan_subnet_input, existing_subnets)
                break
            except ValueError as e:
                print(f"Error: {e}")
        # Save to CSV
        save_subnet_to_csv(os.path.join(BASE_DIR, "subnets.csv"), "server_local_private_subnet", server_lan_subnet)
        ...
        client_names, server_address, port, proto, cipher, data_ciphers = prompt_for_clients()
...
def generate_client_certificates():
    try:
        ...
        openvpn_tunnel_subnet = get_subnet_by_name(os.path.join(BASE_DIR, 'subnets.csv'), "openvpn_tunnel_subnet")
        ...
        logging.error(f"Error during Client Certificate and Configuration generation: {e}")
        print("Failed to generate client certificates and configurations. Check the logs for more details.")
...
def prompt_for_clients():
    ...
    server_address = input(f"Enter the OpenVPN server address [{default_values['server_address']}]: ") or default_values['server_address']
    port = input(f"Enter the OpenVPN server port [{default_values['port']}]: ") or default_values['port']
    proto = input(f"Enter the protocol (tcp/udp) [{default_values['proto']}]: ") or default_values['proto']
    ...
    weakest_cipher = min(selected_ciphers, key=lambda cipher: valid_ciphers.index(cipher))
        ...
        while True:
            ...
            try:
                ...
                if 1 <= selection_int <= len(client_names):
                    ...
                    confirm = input(f"Are you sure you want to revoke client '{client_name_to_revoke}'? (y/n): ").lower()
                    if confirm == 'y':
                        revoke_client(client_name_to_revoke, os.path.join(BASE_DIR, "ca"), os.path.join(BASE_DIR, "ca", "openssl.cnf"), os.path.join(BASE_DIR, "subnets.csv"))
...
def package_server_windows():
    try:
        ...
        print(f"Packaged server files into '{zip_filename}'. You can transfer this zip file to the Windows server and extract it there.")
        ...
        print(f"1.) Copy OpenVPN_Server_Windows.zip to the Windows PC you plan to use as the OpenVPN Server.")
        print(f"2.) Unzip/Extract Zip file to a directory of your choice.")
        print(f"3.) Open Powershell in the directory where you unzipped/extracted the zip file")
        print(f"4.) At the Powershell prompt, run this command powershell -ExecutionPolicy Bypass -File deploy_ovpn_server_on_win10-11.ps1")
        print(f"Once the process is complete, the server directory will be copied to C:\\Program Files\\OpenVPN\\config\\server")
        print(f"It will Start and configure OpenVPN service")
        print(f"It will Enable IP routing in Windows, Enable and start Routing and Remote Access service")
        print(f"Create a scheduled task to run the batch file, which starts OpenVPN Server, at startup")

        # Clean up the temporary directory
        shutil.rmtree(temp_dir)

    except Exception as e:
        print(f"An error occurred while packaging the server for Windows deployment: {e}")
...
def package_server_linux():
    try:
        ...
        with zipfile.ZipFile(zip_filename, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            for root, dirs, files in os.walk(temp_dir):
                for file in files:
                    ...
                    arcname = os.path.relpath(file_path, start=temp_dir)
                    zipf.write(file_path, arcname)
        print(f"Packaged server files into '{zip_filename}'. You can transfer this zip file to the Linux server and extract it there.")
        ...
        print(f"Instructions for Deployment")
        print(f"1.) Copy OpenVPN_Server_Linux.zip to the RHEL/Debian based Linux PC you plan to use as the OpenVPN Server")
        print(f"2.) Unzip/Extract Zip file to a directory of your choice.")
        print(f"3.) Open terminal/shell in the directory where you unzipped/extracted the zip file")
        print(f"4.) At the terminal shell run this command chmod +x deploy_ovpn_server_linux.sh")
        print(f"5.) Then run this command ./deploy_ovpn_server_linux.sh")
        print(f"Once the process is complete, the server directory will be copied to /etc/openvpn/server")
        print(f"It will Start and configure OpenVPN service")
...
def package_server_flexedge():
    try:
        ...
        shutil.copytree(os.path.join(temp_server_dir, folder_to_copy), os.path.join(files_for_sdcard_dir, folder_to_copy))
        ...
        print(f"Packaged server files into '{zip_filename}'. You can transfer this zip file to the FlexEdge device and extract it there.")
        ...
        # Print the deployment instructions
        print(f"Instructions for Deployment")
        print(f"1.) Open the OpenVPN_Server_FlexEdge.zip file and extract it to a directory of your choice.")
        print(f"2.) In the Server directory, you will find the server configuration file server.conf. Upload this configuration file to the FlexEdge device. Instructions in How-to guide.")
        print(f"3a.) Format a MicroSD card with FAT32 file system")
        print(f"3b.) Now you can either copy the files in the files_for_sdcard directory or Use FTP client to move the files to the E-Drive of the FlexEdge device. (Make sure formatted MicroSD card is inserted in the FlexEdge device before boot)")

    except Exception as e:
        print(f"An error occurred while packaging the server for FlexEdge deployment: {e}")
...
def modify_server_conf_for_flexedge(server_conf_path):
    ...
    try:
        ...
        openvpn_status_log = os.path.join(os.path.dirname(server_conf_path), 'openvpn-status.log')
...
def package_client_ewon():
    try:
        ...
        print(f"Updated OVPN file for {client_name} to use Ewon Cosy/Flexy-compatible configuration.")
        ...
        print(f"Packaged client files into '{zip_filename}' for Ewon Cosy/Flexy deployment.")
...
def main():
    ...
    while True:
        ...
        server_initialized = server_config is not None and server_config.get("server_initialized", False)
